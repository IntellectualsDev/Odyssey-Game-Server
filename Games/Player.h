...
//        outputBuffer = CircularBuffer<outputState>(); //size needs to be tick rate * transmission time
    ...
    void updateEntities(FPSClientState& currentState, float dt);
    Vector3 camera_direction(Camera *tcamera);
    void UpdatePlayer(FPSClientState& previousState, FPSClientState& currentState, bool w, bool a, bool s, bool d,Vector2 mouseDelta,bool shoot,bool space,float dt, vector<BoundingBox> &terrainList,vector<BoundingBox> &topBoxVector,bool sprint,bool crouch);
    Camera3D * getCamera();
    Vector3 getHitBox();
